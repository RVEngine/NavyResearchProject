...
void GEARS_API WorldListener_OnObjectCreation(_In_ ObjectHandle_v3 object)
{
    ...
    bool32_t damage_aspect = FALSE;
    Gears::API::DamageAspectAPIv4()->HasAspect(object, &damage_aspect);
    if (damage_aspect != FALSE)
    {
        // Be sure to change this to the name of your component
        Gears::API::DamageAspectAPIv4()->RegisterEvent(object, DamageEventType_v2::kDamageEventType_damage_change_v2, "Quantico_GetData");
        Gears::API::DamageAspectAPIv4()->RegisterEvent(object, DamageEventType_v2::kDamageEventType_destruction_v2, "Quantico_GetData");
        Gears::API::DamageAspectAPIv4()->RegisterEvent(object, DamageEventType_v2::kDamageEventType_hit_v2, "Quantico_GetData");
    }
...
APIResult GEARS_API WeaponSystemAspectListener_OnFire(_In_ ObjectHandle_v3 shooter, _In_ ObjectHandle_v3 shot)
{
    ...
    ComponentDirector::Get().OnFire(shooter, visibilityID, time);
    visibilityID++;
    Faction_v3 faction;
    std::stringstream visbilitystream;
    Gears::API::ORBATAPIv2()->GetFaction(shooter, &faction);
    if (faction == kFaction_opfor)
    {
    //Gears::API::EnvironmentAPIv3()->GetTime(&time);
...
{
    ComponentDirector::Get().enemyVisiblity();
    std::string bluForArray[] = { "rightlead","rightgunner","r3","r4", "mlead","mgunner","m3","m4", "leftlead","leftgunner","l3","l4"};
    std::string opForArray[] = { "erightlead","eright2","eright3","eright4", "emlead","em2","em3","em4", "eleftlead","eleft2","eleft3","eleft4", "efarrightlead","efarright2","efarright3","efarright4"};
    char buffer[256];
    int32_t buffer_size = 256;
    Gears::API::ScriptAPIv3()->GetVariableName(object, buffer, &buffer_size);
    bool found = false;
    for (std::string blufor : bluForArray)
    {
        if (found == false) {
            if (strcmp(buffer, blufor.c_str()) == 0)
            {
                deadBluFor++;
                found = true;
                if (deadBluFor == 12)
                {
                    //end the mission and restart
                    Gears::API::MissionAPIv4()->EndMission();
                    //wait for on mission end callback in missionlistenerapi                   
                    deadBluFor = 0;
                }
            }
        }
    ...
    {
        if (found == false) {
        if (strcmp(buffer, opfor.c_str()) == 0)
        {
            deadOpFor++;
            found = true;
            if (deadOpFor == 12)
    ...
    if (ComponentDirector::Get().elapsed_time >18000)
    {
        Gears::API::MissionAPIv4()->EndMission();
    }
  return kAPIResult_GeneralSuccess;
